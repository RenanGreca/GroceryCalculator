...
public class GroceryItemMO: NSManagedObject {
    ...
    static func new(name: String, position: Int = Int.max) -> GroceryItemMO {
        let item = GroceryItemMO.fetch()
    ...
    static func fetch(uuid: UUID? = nil) -> GroceryItemMO {
        let item:GroceryItemMO
        if  let uuid = uuid,
            let managedItem = GroceryItemMO.fetchManagedWith(id: uuid) {
            item = managedItem
        } else {
            item = NSEntityDescription.insertNewObject(forEntityName: GroceryItemMO.entityName, into: CoreDataHelper.context) as! GroceryItemMO
        }
